...
class Simulator:
    ...
    def run(self, end_time):
        ...
        for _, proc in sorted(self.procs.items()):
            # コルーチンの予備処理をする
            # 処理は最初の yield まで進められ、送信されるデータに対して準備が整う
            # このとき Event が生成される
            first_event = next(proc)
            # PriorityQueue である self.events キューにイベントをそれぞれ入れる
            self.events.put(first_event)

        # main loop of the simulation
        sim_time = 0
        # 現在時刻（sim_time）が終了時刻よりも前の間、回る
        while sim_time < end_time:
            # キューからまだ狩猟していないイベントがなくなったときもループから抜ける
            if self.events.empty():
                print("*** end of events ***")
                break

            # 優先順序つきキューから時刻が最小の Event を取得し、これを current_event（現在のイベント）とする
            current_event = self.events.get()
            # Event データをアンパックする
            # この行でシミュレーションクロック（sim_time）はイベント発生時刻を反映した時刻に更新される
            sim_time, proc_id, previous_action = current_event
            # タクシー番号を示し、その番号に応じてインデントした上で Event を表示する
            print("taxi:", proc_id, proc_id * "    ", current_event)
            # self.procs から処理中のタクシーのコルーチンを取得する
            active_proc = self.procs[proc_id]
            # 前回のイベントを用いて compute_duration(...) を呼び出し、その結果と sim_time を足して値で次にイベントが発生する時刻を定める
            next_time = sim_time + compute_duration(previous_action)
            try:
                # タクシーのコルーチンに時刻を送信する
                # コルーチンは next_event を生成、あるいは完了したら StopIteration を上げる
                next_event = active_proc.send(next_time)
            except StopIteration:
                # StopIteration が上げられたら self.procs からコルーチンを削除する
                del self.procs[proc_id]
            else:
                # それ以外であればキューに next_event を入れる
                self.events.put(next_event)
        else:
            # シミュレーションの終了時刻を過ぎた子によってループが終了したときは、未完のイベントを表示する
